...
def match_bracket(slice_str, i):
    ...
    for m_s, m_r in zip(slice_str[i:], m_br[pos_m_br:len_m_br]):
        if m_s == m_r:
            numb_m_br += 1
            new_pos += 1
            # почему-то если записать if numb_m_br == len_m_br and (slice_str[i-1] == '[' and slice_str[i + numb_m_br] == ']') то выдает ошибку
            if numb_m_br == len_m_br:
                # IndexError для if b_s == m_br[pos_m_br] and open_br == 1 and
                # close_br == 0: на 84 линии
                if slice_str[i - 1] == '[' and slice_str[i + numb_m_br] == ']':
                    dict_m_str[n_str] = 0
                return
        else:
            new_pos = pos_m_br
